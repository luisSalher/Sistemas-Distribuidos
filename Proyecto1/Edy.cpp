...
void Poligono::rotacion(){
   ...
   for(int h=0; h<=nvertices; h++){
       aux1=xf[h];
       aux2=yf[h];
            xf[h] = ((aux1 - coordX) * cos(angulo)) - ((coordY - aux2) * sin(angulo)) + coordX;
            yf[h] = ((coordY - aux2) * cos(angulo)) - ((aux1 - coordX) * sin(angulo)) + coordY;
       xf[h] = (radio * cos(angulo*PI/180))+ coordX;
       yf[h] = (radio * sin(angulo*PI/180)) + coordY;
        }
}
...
vector<int> Poligono::trayectoria(){
    ...
    if(coordX>800||coordX<0||coordY<0||coordY>600){
            lado=2;
            pendiente=pendiente-(2*pendiente);
        }
        if(coordY>600){
            lado=3;
            pendiente=pendiente-2*pendiente;
